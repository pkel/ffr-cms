...
type t =
  { name: string (* full name; goes into commit message *)
...
let nobody = t ~name:"nobody" ~password:(salt 16)
...
type with_handle = string (* login/handle *) * t [@@deriving sexp]

type users = with_handle list [@@deriving sexp]
